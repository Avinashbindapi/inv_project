...
class DBOperation
{
    ...
    {
        $pre_stmt = $this->con->prepare("INSERT INTO `brand`(`brand_name`, `status`) 
        VALUES (?,?)");
        $status = 1;
    ...
    public function addProduct($cid, $bid, $pro_name, $price, $product_stock, $date)
    {
        $status = 1;
        $pre_stmt = $this->con->prepare("INSERT INTO `product` (`cid`, `bid`, `product_name`, `product_price`, `product_stock`, `added_date`, `p_status`) 
        VALUES (?,?,?,?,?,?,?)");
        $pre_stmt->bind_param("iisdisi", $cid, $bid, $pro_name, $price, $product_stock, $date, $status);
        $result = $pre_stmt->execute() or die($this->con->error);
        if ($result) {
